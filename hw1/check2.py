...
class NgramModel:
    ...
    def build(self, corpus, n):
        ...
        for sentence in corpus:
            sentence = ["<s>"] + sentence + ["</s>"]  # Add sentence start and end markers
    ...
    def calculate_smoothed_probability(self, ngram, history, V):
        """
        Calculates the smoothed log probability of an n-gram using Laplace smoothing.
        """
        ngram_freq = self.ngram_count.get(ngram, 0) + 1
        history_freq = self.term_count.get(history, 0) + V
        return np.log(ngram_freq / history_freq)


class CandidatesGenerator:
    """
    Implements spelling correction using n-gram language models and error channel probabilities.
    """
    ...
    def generate_edits(self, word):
        ...
        edits = (
            ...
            [word[:i] + word[j] + word[i + 1:j] + word[i] + word[j + 1:] for i in range(len(word)) for j in range(i + 1, len(word))]
        )
        ...
        for j, word in enumerate(sentence):
            if word not in self.vocabulary:
                ...
                for candidate in candidates:
                    ...
                    prob = np.log(self.channel_prob.get((candidate, word), 0.0001)) - edit_distance
